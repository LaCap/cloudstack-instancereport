...
try:
    ...
    print(
        "It look like elasticsearch-py isn't installed. Please install it"
        "using pip install elasticsearch>=1.0.0,<2.0.0 or pip install "
        "elasticsearch<1.0.0. "
        "See http://elasticsearch-py.readthedocs.org/en/latest/ for more info"
    )
try:
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "This script connect to cloudstack API, list all vms"
            "with details and add  document for each VM in a given ES"
            " cluster index"
        )
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-acssecret",
        help="Cloudstack API user secret",
        required=True,
        type=str,
        dest="acssecret",
    )
    ...
    parser.add_argument(
        "-esnodes",
        help="ES nodes list space separated",
        required=True,
        type=str,
        dest="esnodes",
    )
...
class BaseClient(object):
    ...
    def request(self, command, args):
        ...
        for k in keys:
            params.append(
                k + "=" + urllib.quote_plus(args[k]).replace("+", "%20")
            )
        ...
        signature = base64.b64encode(
            hmac.new(
                self.secret, msg=query.lower(), digestmod=hashlib.sha1
            ).digest()
        )
    ...
    def listVirtualMachines(self, args=None):
        if args is None:
            args = {}
            ...
            body={
                DOCTYPE: {
                    "properties": {
                        ...
                        "memory": {"type": "text"},
                        "name": {"type": "keyword"},
                        "serviceofferingid": {"type": "text"},
                        "serviceofferingname": {"type": "text"},
                        "templateid": {"type": "text"},
                        "templatedisplaytext": {"type": "text"},
                        "zoneid": {"type": "text"},
                        "zonename": {"type": "keyword"},
                        "ipaddress": {"type": "text"},
                        "macaddress": {"type": "text"},
                        "securitygroupid": {"type": "text"},
                        "securitygroupname": {"type": "text"},
